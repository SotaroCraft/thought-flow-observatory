...
def test_resume_skips_completed_immutable_pages(tmp_path: Path) -> None:
    ...
    content_files_after_first = sorted(p.name for p in iter_content_artifact_paths(raw))
    ...
    assert sorted(p.name for p in iter_content_artifact_paths(raw)) != []
    # First page content untouched.
    assert content_files_after_first[0] in {p.name for p in iter_content_artifact_paths(raw)}
...
def test_rerun_completed_partition_does_not_duplicate(tmp_path: Path) -> None:
    ...
    content_count = len(list(iter_content_artifact_paths(raw)))
    ...
    assert len(list(iter_content_artifact_paths(raw))) == content_count
...
def test_multi_country_structured_evidence_survives_raw(tmp_path: Path) -> None:
    ...
    assert result.coverage_status == "success"
    content_path = next(iter_content_artifact_paths(raw))
...
def test_absent_country_evidence_remains_unknown_not_zero(tmp_path: Path) -> None:
    ...
    payload = load_content_payload(next(iter_content_artifact_paths(raw)))
    ...
    prov = pq.read_table(next(iter_run_artifact_paths(raw, result.run_identity)))
